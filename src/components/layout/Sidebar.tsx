...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside
      className={cn(
        'bg-[#091D15] text-[#8A9691] flex flex-col h-screen sticky top-0 shrink-0 select-none overflow-hidden font-sans border-r border-[#14271F] transition-[width] duration-300 ease-in-out relative',
    ...
    >
      ...
      <nav
        className={cn(
          ...
          collapsed ? 'p-2 space-y-4' : 'p-4 space-y-6'
        )}
      >
              ...
              {section.items.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    {!collapsed && <span className="truncate">{item.namaMenu}</span>}
                  </button>
                );
              })}
            </div>
      ...
      <div className={cn('border-t border-[#14271F]', collapsed ? 'p-2' : 'p-4')}>
        {!collapsed ? (
